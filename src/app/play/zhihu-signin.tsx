 ...
 * discarded the moment the handshake completes. There is deliberately no way
 * past it: a failed handshake leaves the player at the same door, by the
 * author's decision. The only remaining recovery is the deployment switch
 * below, which takes a redeploy.
 *
...
function snapshot(): Status {
  return (cached ??= { signedIn: read(SIGNED_IN_KEY), outcome: lastOutcome });
}
...
function note(outcome: Outcome | null): string {
  switch (outcome) {
    case "failed":
      return "刚才没能登录成功，请再试一次。";
    case "declined":
      return "刚才没有完成授权。这个作品需要登录后体验，再授权一次就好。";
    case "unavailable":
      return "知乎登录暂时不可用，请稍后再试。";
    default:
...
/*
 * The hero's action area. Until the player has signed in, the only way forward
 * is the consent page; afterwards this is the ordinary start button it has
 * always been.
 */
export function ZhihuGate({ onStart, startLabel }: { onStart(): void; startLabel: string }) {
  ...
  if (!REQUIRE_SIGN_IN || status.signedIn) {
    return (
      <>
  ...
  return (
    <div className="zhihu-gate">
      <div className="hero-action">
        <a className="btn btn-primary" href="/api/v1/zhihu/oauth/start">
          用知乎账号登录，开始
          <span aria-hidden="true"><ArrowUpRight /></span>
        </a>
        <span className="hero-duration">约 8–10 分钟</span>
...
/** A quiet thank-you at the ending; only reachable by someone already signed in. */
