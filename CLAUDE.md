...
接管或使用已经完成的“人生未定式”网页时，先完整阅读 [前端接入交接文档](docs/CLAUDE_CODE_FRONTEND_ADOPTION_HANDOFF.md)。新版前端已在本工作区实现，包含未提交修改及新增素材；启动应用的目录是本文件所在目录，页面为 `/play`。

下方是早期后端接力任务的历史说明，其中的开发阶段和“精美前端不属于范围”不代表当前状态。前端接入任务以新交接文档和实际源码为依据；保留已有状态引擎、契约与真实内容来源，不要重新生成或覆盖这版网页。
